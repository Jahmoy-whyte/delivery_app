...
import { StyleSheet, SafeAreaView, Text,BackHandler , FlatList, Platform, StatusBar ,Pressable} from 'react-native';
...
const Favoritescreen = ({navigation}) => {
  ...
  //LogBox.ignoreAllLogs();//Ignore all log notifications
     ...
     const [favoritedata, setfavoritedata] = useState({loading:true, data:null, change:null});
  ...
  const [openbottomsheet,
    ...
    setisopen] = useBottomsheet_helper()
        ...
        useEffect(()=>{
          ...
          setfavoritedata(prev => ({...prev,loading:true, change:doc.data().Favorite}))
          })
...
favoritedata.data.length < 1?(
  <Imageandtext title={"No Favorite"} image ={require('../assets/emptycartpic.png')} />
):
